...
| Recommendations — **API-only; replenishment slice** | `/api/recommendations` lists proposals, detail, and history; review/approve/reject/expire/execute endpoints use revisions, idempotency, warehouse/type permissions, and durable lifecycle history. Deterministic replenishment generation revalidates current plan state and creates work through the existing command. A PostgreSQL journey now qualifies concurrent approval and exactly-once normal replenishment work execution. See `Wms.ASP/Controllers/RecommendationsController.cs` and `docs/modernization/RECOMMENDATIONS_GOVERNANCE.md`. | No recommendation UI. Slotting, workload priority, exception resolution, and risk-summary adapters, enabled generation/approval/execution HTTP integration, calibrated backtest/quality monitoring, and production evidence remain #128. |
