...
| Forecasting — **backend-only/incomplete** | Baseline engine/contracts/tests; history adapters, durable runs/jobs, overrides and read API are absent. See `Wms.Application/Forecasting/ForecastingContracts.cs`, `docs/modernization/FORECASTING.md`. | Real warehouse-scoped, versioned forecast reads/jobs remain #126. |
| Anomaly detection — **backend-only/incomplete** | Detector/policy tests exist, but real source adapters, persisted rules/findings, scheduled runs and investigation routes are absent. See `Wms.Application/AnomalyDetection/AnomalyDetectionContracts.cs`, `docs/modernization/ANOMALY_DETECTION.md`. | Persisted, scoped query/investigation lifecycle remains #127. |
