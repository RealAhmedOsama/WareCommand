    ...
    ILogger<NotificationService> logger) : INotificationService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (var recipient in recipients)
        {
            ...
            {
                var decision = ResolvePreference(
                    preferences,
                    recipient,
    ...
    private static PreferenceDecision ResolvePreference(
        ...
        bool mandatory)
    {
        var matches = preferences
            ...
            .OrderByDescending(preference => PreferenceSpecificity(preference, kind, recipient, warehouseId))
            .ThenByDescending(preference => preference.UpdatedAtUtc)
            .FirstOrDefault();
        if (mandatory ||
            matches is null ||
            (matches.IsEnabled &&
             (channel == NotificationChannel.InApp || !IsQuiet(matches, nowUtc))))
        {
            var nextAttempt = matches?.DigestMinutes is > 0 && channel != NotificationChannel.InApp
                ? nowUtc.AddMinutes(matches.DigestMinutes.Value)
                : nowUtc;
    ...
    private static int PreferenceSpecificity(
    ...
    private sealed record PreferenceDecision(
        bool IsEnabled,
